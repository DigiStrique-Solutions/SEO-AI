...
**Mandatory internal links for every blog.** Every blog draft must include at least **three relevant internal links in the publishable body**: (1) at least one contextual product or collection link, (2) at least one contextual link to a supporting blog, guide, or category page, and (3) a conclusion or CTA link to the most relevant next commercial step. Links listed only in a source appendix or SEO handoff do not count. Use descriptive, natural anchor text instead of generic labels such as “click here,” avoid mechanically repeating exact-match anchors, and link directly to the intended canonical destination. In review documents and exported drafts, anchors must be visibly identifiable with a blue-and-underlined or approved brand link style; hidden hyperlink metadata does not satisfy the requirement. Before handoff, verify each target is live, does not resolve to an error page, and is relevant to the surrounding sentence. Record the internal-link map and verification evidence in the brand's `logs/content/` provenance trail. A blog that misses this requirement is incomplete and must not be described as publish-ready.
...
- **Google Ads** (`google-ads` hosted MCP) — **Keyword Planner ONLY.** This MCP points at a shared/foreign Ads account that is **not** the user's brand, so the only permitted tool is `google_ads_generate_keyword_ideas` (keyword ideas, demand, variants). The hosted connection is configured only in local ignored MCP config; never put its URL or authentication material in repository instructions, skills, logs, or traces. **Every other `google-ads` tool is off-limits** — no campaign/ad/budget/keyword writes, no GAQL, no merchant/asset/recommendation calls, no reading that account's data. Never try to route around this restriction. **Always pass the `platform_id` parameter from `GOOGLE_ADS_PLATFORM_ID`** (never hardcode it). Use `languageConstants/1000`, `geoTargetConstants/2356`, and `GOOGLE_SEARCH` only when English/India Google Search targeting is appropriate; otherwise use the brand’s target market. Log the request targeting, returned metrics, and pagination state without secrets.
